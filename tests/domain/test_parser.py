...
from yt_bot.domain.parser import (
    rss_feed, youtube_link, channel_video_ids, format_feed, filter_feed, video_db
    )
...
def test_rss_feed():
    expected = 'https://www.youtube.com/feeds/videos.xml?channel_id=UC41aN7k5-dhOWH2uDM5wU8A'
    assert rss_feed('UC41aN7k5-dhOWH2uDM5wU8A') == expected
...
def test_channel_video_ids():
    expected = {1,2,3}
    orm_class = mock.Mock()
    assert channel_video_ids(FakeSession, orm_class, 1) == expected
...
def test_filter_feed(formatted_feed):
    ...
    assert filter_feed(formatted_feed, video_ids) == expected

def test_video_db(formatted_feed):
    session = mock.Mock()
    video_db(session, 'orm_class', formatted_feed)
